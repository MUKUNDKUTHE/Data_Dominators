...
import { CircleDollarSign, Sparkles, ShieldAlert, BarChart2, TrendingUp, TrendingDown, Minus, Sun, Database, Radio } from 'lucide-react';
...
const HomePage = () => {
  ...
  const actions = [
    { Icon: CircleDollarSign, label: t('checkPrice', language), path: '/recommend' },
    { Icon: Sparkles, label: t('getRecommendation', language), path: '/recommend' },
    { Icon: ShieldAlert, label: t('spoilageCheck', language), path: '/spoilage' },
    { Icon: BarChart2, label: t('marketPrices', language), path: '/markets' },
  ];
    ...
    <div className="pb-24 px-4 pt-16 max-w-lg mx-auto">
      ...
      >
        ...
        <div className="flex items-center gap-3 mt-3 bg-primary-foreground/10 rounded-xl p-3">
          <Sun size={28} className="text-yellow-300 flex-shrink-0" />
          <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {actions.map((action, i) => (
          ...
          >
            <action.Icon size={26} className="text-primary mb-2" strokeWidth={1.8} />
          ...
          {MOCK_PRICES.map((item) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                {(() => { const Icon = TrendIcon(item.trend); return <Icon size={14} className={trendColor(item.trend)} strokeWidth={2.5} />; })()}
              </div>
      ...
      >
        <div className="flex items-center justify-center gap-1.5">
          ...
          <span>Based on 8,80,475+ price records</span>
        </div>
        <div className="flex items-center justify-center gap-1.5 mt-1">
          <Radio size={13} className="text-primary" />
          <span>Live data · Powered by Government Mandi Data</span>
        </div>
      </motion.div>
    </div>
  );
